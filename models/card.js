...
class Card{
    ...
    static async delete(id){
     ...
     if(course.count===1){
         ...
         const b=a-1//count
         car.courses[indx].count=b//изменияем count
         car.courses[indx]//объект измененный
         console.log(indx)
         car.courses.splice(indx,1,car.courses[indx])
         
     }
     card.price -=course.price
     const obj={
        "courses":course.count===1?roc:car.courses ,
         "price":card.price
     }
     return new Promise((res,rej)=>{
